...
- (void)expressBgView{
    ...
    if (_exchange) {
        ...
        _backView.hidden = NO;
        _tableView.frame = CGRM(0, 0, self.width/2, 0);
        [UIView animateWithDuration:0.3 animations:^{
            _tableView.frame = CGRM(0, 0, self.width/2, 44*9);
        }];
        _tableView1.frame = CGRM(self.width/2, 0, self.width/2, 0);
        [UIView animateWithDuration:0.3 animations:^{
            _tableView1.frame = CGRM(self.width/2, 0, self.width/2, 44*9);
        }];
    }
    _exchange = !_exchange;
}
...
- (void)layoutSubviews{
    ...
    _headView.frame = CGRM(0, 0, self.width, 44);
    _titleLabel.frame = CGRM(12, 12, 130, 20);
