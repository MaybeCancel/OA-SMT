...
- (void)layoutSubviews{
    ...
    int imageCount = 5;
    CGFloat imageWidth = (ScreenWidth-15) /imageCount;
    CGFloat imageHeight = imageWidth;
//框子 58
    _titleLabel.frame = RR(15, 15, 100, 15);
    if (self.images.count== 0||!self.images) {
        _adminView.frame = RR(3, CGRectGetMaxY(_titleLabel.frame)+2, imageWidth, imageHeight);
        ...
        for (NSInteger i = 0; i < self.images.count; i ++) {
            ...
            imageView.frame = RR(3 + (imageWidth+3)*(i%imageCount),CGRectGetMaxY(_titleLabel.frame)+2  + (imageWidth+3)*(i/imageCount), imageWidth, imageHeight);
