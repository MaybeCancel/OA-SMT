...
- (void)setUp{
    ...
    self.circleProgress = [[ZXYCircleProgress alloc]initWithFrame:RR(self.width-35-15, 12, 35, 35) progress:0];
    ...
    _lineView.sd_layout.leftSpaceToView(self.contentView, 15).widthIs(ScreenWidth - 15).heightIs(1).bottomSpaceToView(self.contentView, 0);
}
...
- (void)uploadOpeartion:(UIImage*)image{
    NSDictionary* requestDic = @{@"projectId":[NSString stringWithFormat:@"%@",image.projectId],
                          ...
                          @"userId":[NSString stringWithFormat:@"%@",[Default objectForKey:@"userId"]],
                          @"longitude":image.longitude,
                          @"latitude":image.latitude,
                          @"address":[Default objectForKey:@"myLocation"],
    ...
    NSString* urlstring =[CCString getHeaderUrl:@"project/uploadPhoto"];
    [manager POST:urlstring parameters:requestDic constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        [formData appendPartWithFileData:data name:@"1213" fileName:[NSString stringWithFormat:@"%@_%@_%@",image.siteNumber,image.siteName,image.devicePicPart] mimeType:@"image/png"];
...
//        return [urlLink stringByAppendingString:urlString];
//    }
//    return nil;
//}
