...
- (void)customUI{
    ...
    _tips.frame = RR(15, 15, 220, 135);
    [_tips loadInfoFromModel:self.siteModel];
    [self.view addSubview:_tips];
   
    self.littleTip = [[UILabel alloc]initWithFrame:RR(0,ScreenHeight - 15 - 20, ScreenWidth-15, 15)];
    ...
    self.shootingBtn.frame = RR(ScreenWidth / 2- (73/2), ScreenHeight - 73 - 43, 73, 73);
...
- (void) shutterCamera
{
    if (self.secondShoot == NO) {
        ...
        /*这里做照片操作 需要将照片保存传递过去*/
        [LoadingView showAlertHUD:@"保存成功" duration:1];
        /*将照片传到前一个界面*/
...
- (void)shootimage{
    ...
    [self.ImageOutPut captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        ...
        //打水印
        self.image = [UIImage addWatemarkImageWithLogoImage:self.image watemarkImage:[UIImage imageWithUIView:self.littleTip] logoImageRect:self.previewLayer.frame watemarkImageRect:(RR(0,ScreenHeight - 15 - 20, ScreenWidth, 15))];
        
        self.image = [UIImage addWatemarkImageWithLogoImage:self.image watemarkImage:[UIImage imageWithUIView:self.tips] logoImageRect:self.previewLayer.frame watemarkImageRect:RR(15, 15, 220, 135)];
