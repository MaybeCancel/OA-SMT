...
- (void)viewDidLoad {
    ...
    __weak typeof(CompletePicViewController*)weakself = self;
...
- (void)loadDevicePic{
    ...
    BaseRequest* request = [BaseRequest cc_requestWithUrl:[CCString getHeaderUrl:@"project/getPhotoDevices"] isPost:YES Params:@{@"stationId":@"0"}];
...
- (void)loadData{
    BaseRequest* request = [BaseRequest cc_requestWithUrl:[CCString getHeaderUrl:@"project/getStationData"] isPost:YES Params:@{@"userId":[Default objectForKey:@"userId"]}];
    [request cc_sendRequstWith:^(NSDictionary *jsonDic) {
    ...
    [manager getNewLocationHandle:^(CLLocation *newLocation, NSString *newLatitude, NSString *newLongitude) {
        ...
        [geoCoder reverseGeocodeLocation:newLocation completionHandler:^(NSArray<CLPlacemark *> * _Nullable placemarks, NSError * _Nullable error) {
            if(placemarks.count>0){
                ...
                [Default setObject:self.siteModel.myLocation forKey:@"myLocation"];
...
- (void)setUpUI{
    __weak typeof(CompletePicViewController*)weakSel = self;
    self.lists = [[ListsView alloc]initWithTitle:@"设备及照片部位" imageName:@"btn_listD"];
    self.lists.frame = RR(0, 64, self.view.width, 44);
    ...
    self.lists.picPositionAction = ^(NSString* devicePart,NSString *detaiPart,NSString* deviceId,NSString* position) {
        weakSel.siteModel.PicPosition = devicePart;
        weakSel.siteModel.detaiPart = detaiPart;
        weakSel.siteModel.deviceId = deviceId;
        weakSel.siteModel.positionId = position;;
        weakSel.lists.rightString = [NSString stringWithFormat:@"%@[%@]",devicePart,detaiPart];
        [weakSel.tableView reloadData];
    };
    
    // 站号_站名_照片部位[具体角度]
    BaseCellView* cellView = [[BaseCellView alloc]init];
    cellView.frame = RR(0, CGRectGetMaxY(self.lists.frame)+15, self.view.width, 44);
    ...
    cellView.TapHandle = ^{
        ...
        sheet.ClickIndex = ^(NSInteger index) {
            if (index == 2) {//扩容
                self.siteModel.objectType = @"扩容";
                cellView.rightString = @"扩容";
            }else if (index ==1){//新建
                self.siteModel.objectType = @"新建";
                cellView.rightString = @"新建";
            }
        };
    };
    ...
    bottomView.frame = RR(0, self.view.height - 50, self.view.width, 50);
...
- (UIView*)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    ...
    view.frame = RR(0, 0, self.view.width, 44);
    UILabel* label = [[UILabel alloc]init];
    label.frame = RR(13, 15, 130, 14);
    label.textColor = RGBColor(131, 131, 131);
    label.text = @"当前定位信号弱:";
    
    UIButton* btn = [UIButton buttonWithType:UIButtonTypeCustom];
    [btn setTitle:@"去重新定位" forState:UIControlStateNormal];
    btn.frame = RR(CGRectGetMaxX(label.frame), 15, 120, 14);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    site.siteBack = ^(NSString *string, NSInteger tag,NSString* stationId) {
        if (tag == 1) {
            self.siteModel.siteNumber = string;
            self.siteModel.stationId = stationId;
        }else if (tag == 2){
            self.siteModel.siteName = string;
        }
        [self.tableView reloadData];
    };
