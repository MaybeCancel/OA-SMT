...
- (void)layoutSubviews{
    ...
    _anounceImage.frame = RR(12, 12, 27, 16);
    _contentLabel.frame = RR(CGRectGetMaxX(_anounceImage.frame)+10, 12, 200, 16);
    
    CGFloat btnWidth = (self.width - 2) / 3;
    CGFloat btnHeight = (self.height - 4-40) / 4;
    for(NSInteger i = 0; i < _titles.count; i++){
        HomeBtnView* btnView = [self viewWithTag:i + 10];
        btnView.frame = RR(0 + (btnWidth + 1) * (i%3), 40 + (btnHeight + 1) * (i/3), btnWidth, btnHeight);
    }
}
@end
